...
resource "azurerm_key_vault_access_policy" "kv_encryption_set_policy" {
  key_vault_id = var.key_vault_id
  tenant_id    = var.az_tenant_id
...
resource "databricks_secret_acl" "kv_secret_scope_acl_users" {
...
resource "databricks_secret_acl" "kv_secret_scope_acl_admins" {
...
resource "azurerm_databricks_workspace_root_dbfs_customer_managed_key" "datahub_databricks_proj_cmk" {
  workspace_id     = azapi_resource.fsdh_databricks.id
...
resource "azurerm_key_vault_access_policy" "kv_policy_databricks_proj_cmk" {
  key_vault_id = var.key_vault_id
  tenant_id    = var.az_tenant_id
  object_id    = azapi_resource.fsdh_databricks.output.properties.storageAccountIdentity.principalId
