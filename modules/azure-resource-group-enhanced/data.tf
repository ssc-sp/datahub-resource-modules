...
data "azurerm_subnet" "datahub_subnet_ph1" {
  name                 = local.subnet_ph1_name
  virtual_network_name = var.vnet_name
  resource_group_name  = var.vnet_rg
}
data "azurerm_subnet" "datahub_subnet_ph2" {
  name                 = local.subnet_ph2_name
  virtual_network_name = var.vnet_name
  resource_group_name  = var.vnet_rg
}
data "azurerm_subnet" "datahub_subnet_psql" {
  name                 = local.subnet_psql_name
  virtual_network_name = var.vnet_name
  resource_group_name  = var.vnet_rg
}
data "azurerm_subnet" "datahub_subnet_pep" {
  name                 = local.subnet_pep_name
  virtual_network_name = var.vnet_name
  resource_group_name  = var.vnet_rg
}
data "azurerm_subnet" "datahub_subnet_cae" {
  name                 = local.subnet_cae_name
  virtual_network_name = var.vnet_name
  resource_group_name  = var.vnet_rg
}
data "azurerm_subnet" "datahub_subnet_dbrprv" {
  name                 = local.subnet_dbrprv_name
  virtual_network_name = var.vnet_name
  resource_group_name  = var.vnet_rg
}
data "azurerm_subnet" "datahub_subnet_dbrpub" {
  name                 = local.subnet_dbrpub_name
  virtual_network_name = var.vnet_name
  resource_group_name  = var.vnet_rg
}
...
locals {
  ...
  subnet_app_name    = "${var.subnet_name_prefix}_APP-${var.subnet_group_number}-snet"
  subnet_ph1_name    = "${var.subnet_name_prefix}_PH1-${var.subnet_group_number}-snet"
  subnet_ph2_name    = "${var.subnet_name_prefix}_PH2-${var.subnet_group_number}-snet"
  subnet_psql_name   = "${var.subnet_name_prefix}_PGSQL-${var.subnet_group_number}-snet"
  subnet_pep_name    = "${var.subnet_name_prefix}_PEP-${var.subnet_group_number}-snet"
  subnet_cae_name    = "${var.subnet_name_prefix}_CAE-${var.subnet_group_number}-snet"
  ...
  subnet_all = [
    ...
    data.azurerm_subnet.datahub_subnet_ph1.id,
    data.azurerm_subnet.datahub_subnet_ph2.id,
    data.azurerm_subnet.datahub_subnet_cae.id,
    data.azurerm_subnet.datahub_subnet_pep.id,
    data.azurerm_subnet.datahub_subnet_psql.id,
    data.azurerm_subnet.datahub_subnet_dbrprv.id,
    data.azurerm_subnet.datahub_subnet_dbrpub.id
  ]
}
