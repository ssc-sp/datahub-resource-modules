...
resource "azurerm_app_service_certificate_binding" "datahub_ssl_binding" {
  hostname_binding_id = azurerm_app_service_custom_hostname_binding.datahub_app_custom_host.id
  certificate_id      = azurerm_app_service_certificate.datahub_ssl_cert.id
...
resource "azurerm_app_service_certificate" "datahub_ssl_cert" {
  ...
  pfx_blob            = data.azurerm_key_vault_secret.datahub_ssl_cert.value
