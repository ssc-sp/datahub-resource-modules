...
locals {
  ...
  app_service_plan_name   = lower("${var.resource_prefix}-proj-${var.project_cd}-app-plan-${var.environment_name}${local.resource_name_suffix}")
  app_service_name        = lower("${var.resource_prefix}-proj-${var.project_cd}-webapp-${var.environment_name}${local.resource_name_suffix}")
