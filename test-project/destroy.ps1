...
$projectCode=[regex]::match(((Get-Content $pwd\new-project-template.auto.tfvars.json -Raw | grep "project_cd" | Convertto-json)|convertfrom-json),'"project_cd": "(.*)".*').Groups[1].value
$envName=[regex]::match(((Get-Content $pwd\new-project-template.auto.tfvars.json -Raw | grep "environment_name" | Convertto-json)|convertfrom-json),'"environment_name": "(.*)".*').Groups[1].value
